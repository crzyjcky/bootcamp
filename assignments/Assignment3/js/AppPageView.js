...
app.AppPageView = Backbone.View
		.extend({
			...
			initialize : function(options) {
				...
				_.bindAll(this, "onCartButtonClicked");
				_.bindAll(this, "onAddToCartButtonClicked");

				
				$cartButton = $("#cart-button");
				$cartButton.on("click", this.onCartButtonClicked);
				
				
				
				
				$(document).on("click", ".add-to-cart-button", this.onAddToCartButtonClicked);
				...
				this.cartModel = new app.CartModel();
				this.listenTo(this.cartModel, "onCartLineItemAdded", this.onCartLineItemAdded);
				
				var cartComponentView = new app.CartComponentView({
					model : this.cartModel
				});
				cartComponentView.render();
				
				this.listenTo(this.cartModel, "onNCartLineItemChanged", this.onNCartLineItemChanged);
			...
			onProductCollectionReset : function(collection, options) {
				...
				collection.each(function(model, i) {
					...
					var panelComponentView = new app.PanelComponentView({"model" : model});
					$("body").append(panelComponentView.render().el);
					$("#product-" + model.id + " .product-image").on("click", function(){
						
						//var top = window.pageYOffset;
						var top = $(window).scrollTop() + app.Config.headerHeight;
						$("#panel-" + model.id).css("top", top);
						$("#panel-" + model.id).modal();
			...
			onCartLineItemAdded : function(lineItemModel) {
				...
				console.log("onCartLineItem");
				console.log(lineItemModel);
				
				//console.log(productDetailCollection.get(productId));
				
				var alertComponentView = new app.AlertComponentView({model : lineItemModel});
				$("#alert").append(alertComponentView.render().el);
			},
			...
			onCartButtonClicked : function(e) {
			...
			onAddToCartButtonClicked : function(e) {
				
				var productId = $(e.target).data("product-id");
				var productCollection = this.options.productCollection;
				var productModel = productCollection.get(productId);

				var lineItemModel = new app.LineItemModel({
					id : productId, 
					quantity : 1 /* hard code temporary */,
					listPrice : productModel.get("listPrice"),
					price : productModel.get("price"),
					image : productModel.get("image"),
					name : productModel.get("name"),
				});
				
				this.cartModel.addCartLineItem(lineItemModel);
				
				$(e.target).closest(".panel").modal("toggle");
			},
		
			onNCartLineItemChanged : function(n) {
				
				if (n <= 1) {
					$("#n-cart-line-item").html(n + " item");
				} else {
					$("#n-cart-line-item").html(n + " items");
				}
